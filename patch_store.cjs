...
store = store.replace(
  ...
  `let savedModel = localStorage.getItem("geminiModel") || "gemini-1.5-flash";\nif (savedModel.includes("2.5") || savedModel.includes("2.0") || savedModel.includes("3.5") || savedModel.includes("3.8")) {\n  savedModel = "gemini-1.5-flash";\n  localStorage.setItem("geminiModel", savedModel);\n}`
);
